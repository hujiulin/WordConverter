        ...
        private void SaveToToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (null != _htmlFiles && _htmlFiles.Count > 0)
            {
                ...
                if (DialogResult.OK == folderBrowerDialog.ShowDialog())
                {
                    ...
                    _processThread = new System.Threading.Thread(new System.Threading.ThreadStart(this.WordToHTML));
                    timer.Start();
            ...
            if (null != _htmlFiles && _htmlFiles.Count > 0)
            {
                ...
                foreach (var file in _htmlFiles)
                {
                    ...
                    try
                    {
                        ...
                        string outputFilename = Path.Combine(_outputFolder, filename + ".html");
                        if (!File.Exists(outputFilename))
                        {
                            // Initialize word
                            _wordOperation = new WordOperation(file);
                            // Generate HTML
                            _wordOperation.WordToHtml(outputFilename);
                            // Close word
                            _wordOperation.ReleaseDoc();
